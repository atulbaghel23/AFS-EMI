...
<<<<<<< Updated upstream
                    ...
                    {localColConfig.customId && (
                      <td className="px-6 py-4 text-[10px] font-mono font-bold text-text-dim uppercase whitespace-nowrap">
                        {c.customId || "N/A"}
                      </td>
                    )}
                    {localColConfig.mobile && (
                      <td className="px-6 py-4 text-[10px] font-mono font-bold text-text-dim whitespace-nowrap">
                        {c.mobile || "N/A"}
                      </td>
                    )}
                    {localColConfig.email && (
                    ...
                    {localColConfig.gst && (
                      <td className="px-6 py-4 text-[10px] font-mono font-bold text-[#58a6ff] whitespace-nowrap">
                        {c.gst || "N/A"}
                      </td>
                    )}
                    {localColConfig.pan && (
                      <td className="px-6 py-4 text-[10px] font-mono font-bold text-[#f0883e] whitespace-nowrap">
                        {c.pan || "N/A"}
                      </td>
                    )}
                    {localColConfig.bankAcc && (
                    ...
                    {localColConfig.status && (
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-1 h-1 rounded-full ${c.status === "Active" ? "bg-[#3fb950] shadow-[0_0_8px_#3fb950]" : "bg-red-500 shadow-[0_0_8px_#ef4444]"}`}
                          />
                          <span
                            className={`text-[9px] font-black uppercase tracking-widest ${c.status === "Active" ? "text-[#3fb950]" : "text-red-500"}`}
                          >
                            {c.status}
                          </span>
                        </div>
                    ...
                    {localColConfig.type && (
                      <td className="px-6 py-4 text-[9px] font-black text-text-dim uppercase whitespace-nowrap">
                        {c.type}
                      </td>
                    )}
                    {localColConfig.city && (
                      <td className="px-6 py-4 text-[9px] font-bold text-text-dim uppercase whitespace-nowrap">
                        {c.city || "N/A"}
                      </td>
                    )}
                    {localColConfig.pin && (
                      <td className="px-6 py-4 text-[9px] font-mono text-text-dim whitespace-nowrap">
                        {c.pin || "N/A"}
                      </td>
                    )}
                    {localColConfig.address && (
                      <td className="px-6 py-4 text-[9px] font-bold text-text-dim truncate max-w-[150px] whitespace-nowrap">
                        {c.address || "N/A"}
                      </td>
                    )}
                    {localColConfig.overdue && (
                        ...
                        <span className="text-[10px] font-mono font-bold text-red-500">
                          {formatINR(
                            (state.data.loans || [])
                              .filter(
                                (l) =>
                                  (l.customerId?._id || l.customerId) ===
                                    c._id && l.approvalStatus === "Active",
                              )
                              .reduce((sum, l) => {
                                const ovd = (l.schedule || []).filter(
                                  (s) =>
                                    s.status === "Pending" &&
                                    new Date(s.dueDate) < new Date(),
                                );
                                return (
                                  sum + ovd.reduce((s, inst) => s + inst.emi, 0)
                                );
                              }, 0),
                          )}
                        </span>
                      ...
                      <td className="px-6 py-4 text-right sticky right-0 bg-bg-card/95 backdrop-blur-md z-[30]">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button
                            ...
                            className="p-1.5 text-text-dim hover:text-[#f0883e] transition-all"
                          >
                            <Edit3 size={14} />
                          </button>
                          <button
                            onClick={(e) => handleDeleteCustomer(c._id, e)}
                            className="p-1.5 text-text-dim hover:text-red-500 transition-all"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
